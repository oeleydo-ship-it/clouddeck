<div class="app-main !max-w-4xl">
    ...
    <div class="mb-8 flex gap-2" role="progressbar" aria-valuemin="1" aria-valuemax="4" aria-valuenow="{{ $step }}" aria-label="Provisioning step">
        @for($i = 1; $i <= 4; $i++)
            <div class="h-1.5 flex-1 rounded-full transition-colors {{ $i <= $step ? 'bg-cyan-400' : 'bg-slate-100 dark:bg-white/10' }}"></div>
        @endfor
    </div>
    ...
    <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_1px_2px_rgba(0,0,0,0.04)] dark:border-white/10 dark:bg-white/[.04] sm:p-8" wire:loading.class="opacity-60">
        ...
        @elseif($step === 2)
            ...
            <div class="mt-6 grid gap-3">
                @forelse($keys as $key)
                    <label wire:key="ssh-key-{{ $key->id }}" class="flex cursor-pointer items-center gap-4 rounded-2xl border p-4 transition-colors {{ (string) $sshKeyId === (string) $key->id ? 'border-cyan-400 bg-cyan-50 dark:bg-cyan-400/10' : 'border-slate-200 hover:border-slate-300 dark:border-white/10 dark:hover:border-white/20' }}">
                        <input type="radio" wire:model.live="sshKeyId" value="{{ $key->id }}">
        ...
        @else
            <div class="flex items-center gap-3">
                <span class="stat-icon bg-emerald-50 text-emerald-600 dark:bg-emerald-400/10 dark:text-emerald-300">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-5" aria-hidden="true"><path d="M20 6 9 17l-5-5"/></svg>
                </span>
                <div>
            ...
            <div class="mt-6 flex flex-col gap-4 lg:flex-row">
                <dl class="grid min-w-0 flex-1 gap-4 rounded-2xl border border-slate-200 p-5 dark:border-white/10 sm:grid-cols-2">
                    @foreach([
                        'Name' => $name,
                        'Hostname' => $hostname,
                        ...
                        <div wire:key="review-{{ \Illuminate\Support\Str::slug($label) }}">
                            <dt class="text-xs uppercase tracking-wide muted">{{ $label }}</dt>
                            <dd class="mt-1 font-medium heading">{{ $value }}</dd>
                        </div>
                    @endforeach
                    <div class="sm:col-span-2">
                        <dt class="text-xs uppercase tracking-wide muted">Type</dt>
                        <dd class="mt-1"><span class="badge bg-cyan-50 text-cyan-700 dark:bg-cyan-400/10 dark:text-cyan-300">Managed server</span></dd>
                    </div>
                </dl>

                <div class="w-full shrink-0 rounded-2xl border border-cyan-200 bg-cyan-50/60 p-5 dark:border-cyan-400/20 dark:bg-cyan-400/[.06] lg:w-72">
                    ...
                    <ul class="mt-4 flex flex-col gap-2 text-sm muted">
                        @foreach([
                            'Billed monthly via Stripe Checkout',
                            'Counts toward managed servers, not BYOS',
                            'Provisions after payment confirms',
                        ] as $benefit)
                            <li class="flex items-start gap-2" wire:key="benefit-{{ $loop->index }}">
                                <span class="mt-0.5 font-semibold leading-none text-emerald-600 dark:text-emerald-400" aria-hidden="true">✓</span>
                                <span>{{ $benefit }}</span>
                            </li>
                        @endforeach
                    </ul>
        ...
        @foreach(['region','size','image','sshKeyId','name','hostname','quota','billing'] as $field)
        ...
        <div class="mt-8 flex justify-between">
            ...
            @else
                <button type="button" wire:click="deploy" wire:loading.attr="disabled" class="button-primary" @disabled($catalogError !== '')>
                    @if(($customerPrice ?? 0) > 0)
                        Pay ${{ number_format($customerPrice, 2) }}/mo &amp; deploy
                    @else
