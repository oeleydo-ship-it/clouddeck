@php
    $backupDisks = $backupDisks ?? collect(config('filesystems.disks'))
        ->reject(fn (array $disk) => ($disk['visibility'] ?? null) === 'public')
        ->keys();
    $defaultBackupDisk = config('remote_management.database_backup_disk', 'local');
...
<div x-cloak x-show="tab==='backups'" class="mt-6 space-y-6">
    <div class="grid gap-6 lg:grid-cols-[400px_1fr]">
        <form method="POST" action="{{ route('backup-policies.store', $server) }}" class="panel h-fit">@csrf
            ...
            <label class="mt-4 block text-sm">Recovery point<select class="field" name="type" x-model="backupType"><option value="database">Database export</option>@if($server->provider_id)<option value="snapshot">Provider snapshot</option>@endif</select></label>
            ...
            <label x-show="backupType==='database'" class="mt-4 block text-sm">Storage disk<select class="field" name="disk">@foreach($backupDisks as $disk)<option value="{{ $disk }}" @selected($disk === $defaultBackupDisk)>{{ $disk }}{{ $disk === $defaultBackupDisk ? ' (default)' : '' }}</option>@endforeach</select></label>
    ...
    <section class="panel">
        <div class="flex flex-wrap items-end justify-between gap-4">
            <div>
                <h2 class="font-semibold">Provider snapshots</h2>
                @if($server->provider_id)
                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Full Droplet disk snapshots at DigitalOcean. Restoring replaces the server disk — type the hostname to authorize it.</p>
                @else
                    <p class="mt-1 text-sm text-amber-700 dark:text-amber-300">Provider snapshots require a cloud Droplet. Connected custom servers can use database export policies instead.</p>
                @endif
            </div>
            @if($server->provider_id)
