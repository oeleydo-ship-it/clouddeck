    ...
    <div x-cloak x-show="tab==='monitoring'" class="mt-6 space-y-6">
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            @foreach([['CPU',$latestMetric?->cpu_percent],['Memory',$latestMetric?->memory_percent],['Disk',$latestMetric?->disk_percent],['Load',$latestMetric?->load_average]] as [$label,$value])
                <div class="panel p-5"><p class="text-sm text-slate-500 dark:text-slate-400">{{ $label }}</p><p class="mt-2 text-3xl font-semibold">{{ $value ?? '-' }}{{ $value !== null && $label !== 'Load' ? '%' : '' }}</p></div>
            @endforeach
        </div>
        @php
