...
<div class="app-main" x-data="managedTabs({ tab: @js($initialServerTab), keys: @js($serverTabKeys) })" @submit.capture="ensureTab($event)">
    ...
    @if($server->status === \App\Enums\ServerStatus::AwaitingPayment)
        <div class="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-400/20 dark:bg-amber-400/10">
            <p class="text-sm text-amber-800 dark:text-amber-100">This managed server is waiting for payment of <strong>${{ number_format((float) data_get($server->metadata, 'customer_price_monthly', 0), 2) }}/mo</strong>. Provisioning starts after Stripe confirms the charge.</p>
            <form method="POST" action="{{ route('servers.managed.checkout', $server) }}" class="mt-3">@csrf
                <button class="button-primary">{{ data_get($server->metadata, 'stripe_checkout_session_id') ? 'Check payment / complete checkout' : 'Complete payment' }}</button>
            </form>
        </div>
    @endif
    ...
    <div x-show="tab==='databases'" class="mt-6 grid gap-6 lg:grid-cols-[380px_1fr]"><form method="POST" action="{{ route('databases.store',$server) }}" class="panel h-fit">@csrf<h2 class="font-semibold">Create database</h2><label class="mt-4 block text-sm">Engine<select class="field" name="engine"><option value="mysql">MySQL / MariaDB</option><option value="postgresql">PostgreSQL</option></select></label><label class="mt-4 block text-sm">Database name<input class="field" name="name" placeholder="application"></label><label class="mt-4 block text-sm">Username<input class="field" name="username" placeholder="application_user"></label><label class="mt-4 block text-sm">Attach to site<select class="field" name="site_id"><option value="">None</option>@foreach($server->sites as $site)<option value="{{ $site->id }}">{{ $site->domain }}</option>@endforeach</select></label><button class="button-primary mt-5">Create database</button></form><div class="space-y-3">@forelse($server->databases as $database)<article class="panel"><div class="flex flex-wrap items-start justify-between gap-4"><div><h3 class="font-medium">{{ $database->name }}</h3><p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ $database->engine }} · {{ $database->username }} · <span class="capitalize">{{ $database->status }}</span></p>@if($database->site)<p class="mt-1 text-xs muted">Attached to <a class="link-action" href="{{ route('sites.show', $database->site) }}">{{ $database->site->domain }}</a></p>@else<p class="mt-1 text-xs muted">Not attached to a site</p>@endif @if($database->failure_reason)<p class="mt-2 text-xs text-rose-600 dark:text-rose-300">{{ $database->failure_reason }}</p>@endif</div></div>@if(in_array($database->status, ['ready', 'failed'], true))<form method="POST" action="{{ route('databases.update', $database) }}" class="mt-4 flex flex-wrap items-end gap-2 border-t border-slate-100 pt-4 dark:border-white/5">@csrf @method('PATCH')<label class="min-w-[12rem] flex-1 text-sm">Attach to site<select class="field mt-1" name="site_id"><option value="">None</option>@foreach($server->sites as $site)<option value="{{ $site->id }}" @selected($database->site_id === $site->id)>{{ $site->domain }}</option>@endforeach</select></label><button class="button-secondary">Save attachment</button></form>@endif<form method="POST" action="{{ route('databases.destroy',$database) }}" class="mt-4 flex flex-wrap items-end gap-2 border-t border-slate-100 pt-4 dark:border-white/5" onsubmit="return confirm('Permanently delete database {{ $database->name }}? All tables and data will be dropped on the server. This cannot be undone or recovered.')">@csrf @method('DELETE')<label class="min-w-[12rem] flex-1 text-xs text-slate-500 dark:text-slate-400">Type {{ $database->name }} to confirm deletion<input class="field mt-1" name="confirmation" placeholder="{{ $database->name }}" autocomplete="off"></label><button class="button-secondary !text-rose-600 dark:!text-rose-300">Delete database</button></form></article>@empty<div class="panel text-center text-slate-500 dark:text-slate-400">No managed databases.</div>@endforelse</div></div>
