@extends('layouts.app')
@section('content')
<div class="app-main" x-data="{ tab: 'monitoring', backupType: 'database', frequency: 'daily', keys: ['monitoring','databases','backups','cron','workers','services'], init() { const h = location.hash.replace('#',''); if (this.keys.includes(h)) { this.tab = h } this.$watch('tab', v => history.replaceState(null, '', '#' + v)) } }">
    <div><a href="/dashboard" class="link-action">← Dashboard</a><h1 class="page-title">{{ $server->name }}</h1><p class="mt-2 text-sm text-slate-500 dark:text-slate-400">{{ $server->public_ip }} · {{ $server->region }} · <span class="capitalize">{{ $server->status->value }}</span></p></div>
    ...
    <div x-cloak x-show="tab==='cron'" class="mt-6 grid gap-6 lg:grid-cols-[380px_1fr]"><form method="POST" action="{{ route('cron-jobs.store',$server) }}" class="panel h-fit">@csrf<h2 class="font-semibold">Add server cron job</h2><p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Runs as root, unattached to a site. Site schedulers live on each site's Cron tab.</p><label class="mt-4 block text-sm">Name<input class="field" name="name" placeholder="Laravel scheduler"></label><label class="mt-4 block text-sm">Expression<input class="field font-mono" name="expression" value="* * * * *"></label><label class="mt-4 block text-sm">Command<input class="field font-mono text-xs" name="command" placeholder="cd /var/www/app/current && php artisan schedule:run"></label><button class="button-primary mt-5">Add cron</button></form><div class="space-y-3">@forelse($server->cronJobs->whereNull('site_id') as $cron)<article class="panel"><div class="flex justify-between gap-4"><div><h3>{{ $cron->name }}</h3><code class="mt-2 block text-xs text-slate-500 dark:text-slate-400">{{ $cron->expression }} · {{ $cron->command }}</code></div><div class="flex gap-3"><form method="POST" action="{{ route('cron-jobs.toggle',$cron) }}">@csrf @method('PATCH')<button class="link-action">{{ $cron->enabled?'Disable':'Enable' }}</button></form><form method="POST" action="{{ route('cron-jobs.destroy',$cron) }}">@csrf @method('DELETE')<button class="text-sm text-rose-600 dark:text-rose-300">Delete</button></form></div></div></article>@empty<div class="panel text-center text-slate-500 dark:text-slate-400">No server-level cron jobs.</div>@endforelse</div></div>
    ...
    <div x-cloak x-show="tab==='services'" class="mt-6"><div class="panel"><h2 class="font-semibold">Service controls</h2><div class="mt-5 flex flex-wrap gap-3">@foreach(['nginx:test'=>'Test Nginx','nginx:reload'=>'Reload Nginx','nginx:restart'=>'Restart Nginx','php:reload'=>'Reload PHP','php:restart'=>'Restart PHP','supervisor:restart'=>'Restart workers','redis:restart'=>'Restart Redis','mysql:restart'=>'Restart MySQL'] as $type=>$label)<form method="POST" action="{{ route('server-operations.store',$server) }}">@csrf<input type="hidden" name="type" value="{{ $type }}"><button class="button-secondary">{{ $label }}</button></form>@endforeach</div></div>
        ...
        <div class="mt-5 space-y-3">@foreach($server->operations as $operation)<article class="panel"><div class="flex justify-between"><span>{{ $operation->type }}</span><span class="text-sm capitalize">{{ $operation->status }}</span></div>@if($operation->output)<pre class="mt-3 max-h-48 overflow-auto whitespace-pre-wrap rounded-xl bg-slate-100 dark:bg-black/40 p-3 text-xs">{{ $operation->output }}</pre>@endif</article>@endforeach</div></div>
    <div x-cloak x-show="tab==='monitoring'" class="mt-6 space-y-6">
        ...
        @endphp
        <section class="panel"><div class="flex flex-wrap items-center justify-between gap-3"><div><h2 class="font-semibold">Resource history</h2><p class="mt-1 text-sm text-slate-500 dark:text-slate-400">The latest {{ $chartMetrics->count() }} one-minute samples</p></div><div class="flex gap-4 text-xs"><span class="text-cyan-600 dark:text-cyan-300">CPU</span><span class="text-violet-600 dark:text-violet-300">Memory</span><span class="text-amber-600 dark:text-amber-300">Disk</span></div></div>@if($chartMetrics->count())<svg class="mt-5 h-52 w-full" viewBox="0 0 600 160" role="img" aria-label="CPU, memory, and disk usage history"><path d="M0 10H600 M0 80H600 M0 150H600" stroke="currentColor" class="text-slate-200 dark:text-white/10"/><polyline points="{{ $chartPoints('cpu_percent') }}" fill="none" stroke="rgb(103 232 249)" stroke-width="3"/><polyline points="{{ $chartPoints('memory_percent') }}" fill="none" stroke="rgb(196 181 253)" stroke-width="3"/><polyline points="{{ $chartPoints('disk_percent') }}" fill="none" stroke="rgb(252 211 77)" stroke-width="3"/></svg>@else<p class="mt-5 text-sm text-slate-500 dark:text-slate-400">Waiting for the first signed sample.</p>@endif</section>
        <section class="panel">
