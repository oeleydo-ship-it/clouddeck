    ...
    <div class="relative mx-auto max-w-7xl px-5 py-20 lg:py-28">
        ...
        <p class="mt-6 max-w-2xl text-lg muted">
            @if($managedServersEnabled)
                {{ $platform }} is a SaaS panel for provisioning managed servers, connecting your own VPS, deploying sites, and handling day-to-day ops — so you ship apps instead of wrestling with the server.
            @else
                {{ $platform }} is a SaaS panel for provisioning servers, deploying sites, and handling day-to-day tasks — on infrastructure you own and bill directly with your provider.
            @endif
        </p>
    </div>
</section>
    ...
    <div class="grid gap-12 lg:grid-cols-3">
        @foreach([
            ...
            $managedServersEnabled
                ? ['Managed or bring-your-own', "Need a server fast? We provision a managed VPS on {$platform}'s cloud account. Prefer your own provider? Connect DigitalOcean, Hetzner, or any Ubuntu VPS and keep billing with them."]
...
<section class="border-y border-slate-200 bg-slate-50 py-20 dark:border-white/10 dark:bg-white/[.02]">
    <div class="mx-auto max-w-7xl px-5">
        ...
        <div class="mt-12 grid gap-8 md:grid-cols-2 lg:grid-cols-4">
            @foreach([
                $managedServersEnabled
                    ? ['Choose how you host', 'Provision a managed server through us, or connect a VPS you already pay for — same panel either way.']
