...
Nginx revisions control request size, static-asset caching, the optional `www` hostname, and whether the site may be embedded in cross-origin iframes. By default the virtual host sends `X-Frame-Options: SAMEORIGIN` (clickjacking protection). Enable **Allow embedding in iframes on other sites** when this site hosts a chat widget or similar embed that another domain loads via Admin → Insert code; that omits `X-Frame-Options` so the iframe can render. The generated virtual host still retains the other security headers, protected dotfiles, Laravel front-controller routing, access logs, and the dedicated FPM socket. An active Let's Encrypt certificate is included automatically. Enabling `www` on an SSL site is rejected unless the active certificate contains that hostname. Nginx configuration is backed up and `nginx -t` must pass before reload; a failed test restores the previous file.
